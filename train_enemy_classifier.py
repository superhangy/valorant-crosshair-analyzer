"""
Train a general good-vs-bad ("is this actually an enemy") classifier on the
box crops from extract_enemy_classifier_dataset.py
(dataset_enemy_classifier/{good,bad}/). Same architecture as
train_ally_classifier.py / train_gunmodel_classifier.py (frozen ResNet18
backbone with layer4 unfrozen at a lower LR, class-weighted loss, heavy
augmentation) -- broader label set (every reviewed bad category, not just
one), so this is the model to reach for when the question is "would a human
reviewer keep this candidate at all," not "is this specifically a
teammate/gun-viewmodel."

Usage:
    python train_enemy_classifier.py
"""
...
WEIGHTS_OUT = REPO_ROOT / "enemy_classifier_head.pt"
...
def evaluate(model, loader, device):
    model.eval()
    tp = fp = fn = tn = 0  # positive class = "bad" (index 1)
    correct = total = 0
    with torch.no_grad():
        for x, y in loader:
...
def main():
    ...
    print(f"dataset: good={n_good} bad={n_bad} total={len(items)}")
    if len(items) < 20:
        raise SystemExit("Too few images -- run extract_enemy_classifier_dataset.py first")
    ...
    n_train_bad = sum(1 for _, l in train_items if l == 1)
    weights = torch.tensor(
        [len(train_items) / (2 * n_train_good), len(train_items) / (2 * n_train_bad)],
        dtype=torch.float32,
    ).to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"epoch {epoch:2d}  train_loss={total_loss/len(train_items):.4f}  "
              f"val_acc={val_acc:.3f}  val_precision(bad)={val_prec:.3f}  "
              f"val_recall(bad)={val_rec:.3f}")
