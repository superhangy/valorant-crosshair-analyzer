"""
Pre-flag pass for review_engagements.py: runs the trained enemy_classifier
(enemy_classifier_head.pt, see train_enemy_classifier.py) over every raw
engagement frame in a folder and writes a <folder>_enemy_predictions.csv of
"bad" probability scores (general good-vs-bad, all categories combined --
see predict_teammate_prob.py / predict_gunmodel_prob.py for the two
category-specific versions). Does NOT touch engagements.csv or drop
anything -- pure pre-sort convenience, every frame still gets manually
reviewed.

Usage:
    python predict_enemy_prob.py engagements_<slug> [<folder> ...]
"""
...
WEIGHTS_PATH = REPO_ROOT / "enemy_classifier_head.pt"
...
def predict_prob(model, device, crop_bgr):
    ...
    return probs[0, 1].item()  # index 1 = "bad"


def main():
    if len(sys.argv) < 2:
        raise SystemExit("Usage: python predict_enemy_prob.py <engagements_folder> [<folder> ...]")

    if not WEIGHTS_PATH.exists():
        raise SystemExit(f"No trained weights at {WEIGHTS_PATH} -- run train_enemy_classifier.py first")
    ...
    for folder_arg in sys.argv[1:]:
        ...
        out_path = folder.parent / f"{folder.name}_enemy_predictions.csv"
        ...
        with open(out_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["filename", "bad_prob"])
