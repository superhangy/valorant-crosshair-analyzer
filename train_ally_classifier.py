...
WEIGHTS_OUT = REPO_ROOT / "ally_classifier_head.pt"
...
def evaluate(model, loader, device):
    model.eval()
    tp = fp = fn = tn = 0  # positive class = "teammate" (index 1)
    correct = total = 0
    with torch.no_grad():
        for x, y in loader:
...
def main():
    ...
    n_teammate = sum(1 for _, l in items if l == 1)
    print(f"dataset: good={n_good} teammate={n_teammate} total={len(items)}")
    if len(items) < 20:
        raise SystemExit("Too few images -- run extract_ally_classifier_dataset.py first")
    ...
    # class-weighted loss to counter the 57/101 imbalance
    n_train_good = sum(1 for _, l in train_items if l == 0)
    n_train_teammate = sum(1 for _, l in train_items if l == 1)
    weights = torch.tensor(
        [len(train_items) / (2 * n_train_good), len(train_items) / (2 * n_train_teammate)],
        dtype=torch.float32,
    ).to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"epoch {epoch:2d}  train_loss={total_loss/len(train_items):.4f}  "
              f"val_acc={val_acc:.3f}  val_precision(teammate)={val_prec:.3f}  "
              f"val_recall(teammate)={val_rec:.3f}")
