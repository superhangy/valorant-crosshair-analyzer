"""
Pre-flag pass for review_engagements.py: runs the trained ally_classifier
(ally_classifier_head.pt, see train_ally_classifier.py) over every raw
engagement frame in a folder and writes a <folder>_predictions.csv of
teammate-probability scores. Does NOT touch engagements.csv or drop
anything -- review_engagements.py reads this file (if present) to sort
likely-teammate frames first / show the score, but every frame still gets
manually reviewed. Pure pre-sort convenience, not a filter.

Usage:
    python predict_teammate_prob.py engagements_<slug> [<folder> ...]
"""
...
WEIGHTS_PATH = REPO_ROOT / "ally_classifier_head.pt"
...
def predict_prob(model, device, crop_bgr):
    # crop_bgr is an OpenCV BGR ndarray -- convert to RGB PIL for torchvision transforms.
    import cv2
    ...
    return probs[0, 1].item()  # index 1 = "teammate"


def main():
    if len(sys.argv) < 2:
        raise SystemExit("Usage: python predict_teammate_prob.py <engagements_folder> [<folder> ...]")

    if not WEIGHTS_PATH.exists():
        raise SystemExit(f"No trained weights at {WEIGHTS_PATH} -- run train_ally_classifier.py first")
    ...
    for folder_arg in sys.argv[1:]:
        ...
        out_path = folder.parent / f"{folder.name}_predictions.csv"
        ...
        with open(out_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["filename", "teammate_prob"])
